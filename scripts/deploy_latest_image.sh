result=$(doctl registry repository list-tags saleor-smtp | grep latest | grep -o sha256:.* | xargs -i kubectl set image deployment/saleor-app-smtp-test saleor-app-smtp-test=registry.digitalocean.com/whitewhale/saleor-smtp@{} -n whitewhale)
echo $result
