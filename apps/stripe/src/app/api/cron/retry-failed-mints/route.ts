 ...
 *
 * Returns a JSON summary `{processed, succeeded, failedRetries, finalFailures, totalErrors}`.
 *
 * The route deliberately NEVER throws past the auth check — partial failures
 * are surfaced through the summary and via Sentry, never via a 5xx (Vercel
...
const productionDeps = (): RetryCronDeps => ({
  apl: saleorApp.apl,
 ...
 */
const verifyCronAuth = (request: Request, expectedSecret: string | undefined): Response | null => {
  if (!expectedSecret) {
...
export const __testing = { verifyCronAuth };
