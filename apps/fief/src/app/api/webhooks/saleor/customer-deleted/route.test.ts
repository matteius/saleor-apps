 ...
 *
 * T29 — Saleor `CUSTOMER_DELETED` webhook route handler test.
 *
 ...
 * into `webhook-definition.ts`); the dedicated `verify-signature.test.ts` suite
 * covers the cryptography. This file only asserts the route's branching:
 *
 *   - kill switch (T54) → 503 (no enqueue)
 *   - origin marker `"fief"` (T13) → 200 + skipped (no enqueue, no Fief I/O)
 *   - happy path (clean payload) → 200 + enqueued (eventType
 *     `saleor.customer_deleted`)
  ...
  vi.fn(() =>
    Promise.resolve(
      ok({
        ...
        connectionId: "conn-1",
        eventType: "saleor.customer_deleted",
        eventId: "evt-1",
        payload: {},
        attempts: 0,
...
vi.mock("@saleor/app-sdk/handlers/next-app-router", () => ({
  SaleorAsyncWebhook: vi.fn().mockImplementation(() => ({
    ...
    getWebhookManifest: () => ({ name: "Customer Deleted" }),
  })),
}));
...
const buildRequest = (): Request => {
  const url = "https://app.test/api/webhooks/saleor/customer-deleted";
  const req = new Request(url, {
    method: "POST",
    body: JSON.stringify({ webhook: "saleor", event: "CUSTOMER_DELETED" }),
...
describe("Saleor CUSTOMER_DELETED route — T29", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("happy path: enqueues the event with eventType 'saleor.customer_deleted' and returns 200", async () => {
    ...
    expect(args.eventType).toBe("saleor.customer_deleted");
