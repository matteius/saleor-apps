...
interface MongoRecordedTransaction {
  ...
  /*
   * Added in upstream after the fork was first carved off. Optional in the schema so legacy
   * rows (written without this field) can still be read.
   */
  saleorSchemaVersion?: { major: number; minor: number };
}
...
export class MongodbTransactionRecorderRepo implements TransactionRecorderRepo {
  ...
  async recordTransaction(
    ...
    try {
      ...
      const [schemaMajor, schemaMinor] = transaction.saleorSchemaVersion;
      const mongoTransaction: MongoRecordedTransaction = {
        ...
        saleorSchemaVersion: { major: schemaMajor, minor: schemaMinor },
      };
  ...
  async getTransactionByStripePaymentIntentId(
    ...
    try {
      ...
      /*
       * Legacy rows (written before saleorSchemaVersion was added) default to the schema we're
       * running today. Production transactions are short-lived; an in-flight payment row is
       * always against the current Saleor server version.
       */
      const schema = mongoTransaction.saleorSchemaVersion ?? { major: 3, minor: 23 };
      const recordedTransaction = new RecordedTransaction({
        ...
        saleorSchemaVersion: [schema.major, schema.minor],
      });
