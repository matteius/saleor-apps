...
import { appRootTracer as appInternalTracer } from "../../lib/app-root-tracer";
import { createLogger } from "../../logger";

const ValueError = BaseError.subclass("ValueError");

/**
 * Wrap createSaleorApiUrl from the shared domain package — it throws on invalid input,
 * but the existing call sites here expect to handle the value path explicitly.
 */
const tryCreateSaleorApiUrl = (raw: string): SaleorApiUrl | null => {
  try {
    return createSaleorApiUrl(raw);
  } catch {
    return null;
  }
};
...
export class MongoAPL implements APL {
  ...
  async get(saleorApiUrl: SaleorApiUrl | string): Promise<AuthData | undefined> {
    const saleorApiUrlParsed = tryCreateSaleorApiUrl(saleorApiUrl);

    if (!saleorApiUrlParsed) {
    ...
    return this.tracer.startActiveSpan("MongoAPL.get", async (span) => {
      try {
        ...
        const result = await this.collection!.findOne({
          saleorApiUrl: saleorApiUrlParsed,
        });
