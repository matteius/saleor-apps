 ...
 *
 * T27 — Saleor `CUSTOMER_UPDATED` webhook route handler test.
 *
 * Mirrors T26 exactly — only the queue eventType + log channel differ. Mocks
 * the SDK's `SaleorAsyncWebhook.createHandler(...)` so we exercise the route's
 * enqueue + skip paths without standing up JWKS / APL / Mongo.
 *
 * Coverage:
 ...
 *   - enqueue failure → 5xx (Saleor retries)
 */
  ...
  vi.fn(() =>
    Promise.resolve(
      ok({
        ...
        connectionId: "conn-1",
        eventType: "saleor.customer_updated",
        eventId: "evt-1",
        payload: {},
        attempts: 0,
...
vi.mock("@saleor/app-sdk/handlers/next-app-router", () => ({
  SaleorAsyncWebhook: vi.fn().mockImplementation(() => ({
    ...
    getWebhookManifest: () => ({ name: "Customer Updated" }),
  })),
}));
...
const buildRequest = (): Request => {
  const url = "https://app.test/api/webhooks/saleor/customer-updated";
  const req = new Request(url, {
    method: "POST",
    body: JSON.stringify({ webhook: "saleor", event: "CUSTOMER_UPDATED" }),
...
describe("Saleor CUSTOMER_UPDATED route — T27", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("happy path: enqueues the event and returns 200", async () => {
    ...
    expect(args.eventType).toBe("saleor.customer_updated");
    expect(args.saleorApiUrl).toBe(SALEOR_API_URL);
    /*
     * Synthetic event id format mirrored from T26: distinguishing suffix is
     * `customer_updated`, not `customer_created`.
     */
    expect(args.eventId).toBe(`saleor:${SALEOR_API_URL}:VXNlcjox:customer_updated`);
  });
